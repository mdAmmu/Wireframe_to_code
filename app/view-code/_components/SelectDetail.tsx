...
const SelectDetail = ({ record, regenerateCode, isReady }: any) => {
  return record && (
    <div className='p-5 bg-gray-100 rounded-lg'>
      <h2 className='font-bold my-2'>Wireframe</h2>
      <Image src={record?.imageUrl} alt='Wireframe' width={300} height={400}
        className='roundd-lg object-contain h-[200px] w-full border border-dashed p-2 bg-white' />
      ...
      <Button onClick={regenerateCode} disabled={isReady} className='mt-7 w-full bg-[#372f93]'> <RefreshCcw/>Regenerate Code</Button>
    </div>
  )
}
